...
bool applyRuntimeBalanceValues(const std::unordered_map<std::string, std::string>& values, RuntimeBalance& outBalance, std::string& outError)
{
    ...
    if (!setInt("enemy_min_dug_tiles", loaded.enemyMinDugTiles)) return false;
    if (!setInt("enemy_guarantee_dug_tiles", loaded.enemyGuaranteeDugTiles)) return false;
    ...
    if (!setFloat("enemy_detected_vision_multiplier", loaded.enemyDetectedVisionMultiplier)) return false;
    if (!setFloat("enemy_ring_slow_bite_multiplier", loaded.enemyRingSlowBiteMultiplier)) return false;
    if (!setFloat("enemy_ring_slow_bite_duration", loaded.enemyRingSlowBiteDuration)) return false;
    ...
    if (!setInt("dig_item_guarantee_dug_tiles", loaded.digItemGuaranteeDugTiles)) return false;
    ...
    loaded.enemyMinDugTiles = std::max(1, loaded.enemyMinDugTiles);
    loaded.enemyGuaranteeDugTiles = std::max(loaded.enemyMinDugTiles, loaded.enemyGuaranteeDugTiles);
    ...
    loaded.enemyDetectedVisionMultiplier = std::max(1.0f, loaded.enemyDetectedVisionMultiplier);
    loaded.enemyRingSlowBiteMultiplier = std::clamp(loaded.enemyRingSlowBiteMultiplier, 0.05f, 1.0f);
    loaded.enemyRingSlowBiteDuration = std::max(0.0f, loaded.enemyRingSlowBiteDuration);
    ...
    loaded.digItemGuaranteeDugTiles = std::max(loaded.digItemMinDugTiles, loaded.digItemGuaranteeDugTiles);
